...
const BASE_PATH = "./circuits/polycircuit/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/polycircuit.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/PolycircuitVerifier.sol:PolycircuitVerifier");
